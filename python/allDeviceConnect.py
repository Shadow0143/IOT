    ...
    def handleNotification(self, handle, data):
        ...
        try:
            if len(data) > 1:
                ...
                # spo2data = {"Device Name": selected_device_name,'Heart Rate': data[8], 'SPO2': data[7],'Movement': data[16],'BPM PT' : data[17]} 

                # local_date_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

                # print(local_date_time)
                query = {'device_id': selected_device_name,'type':'heart_rate','reading': data[8]} 

                print(query)     
                response = requests.post("http://192.46.212.177:3020/api/iot-request/create", data=query)
